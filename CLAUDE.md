...
This is the whole reason passwords are allowed in the CRM at all: nightly `pg_dump` backups leave the machine, and a dump of ciphertext is worthless. So **never add server-side decryption, password search, or validation** — any of those needs the key on the server and re-creates the exact problem this design avoids. Two consequences are load-bearing, not bugs: a forgotten passphrase means permanently lost passwords (nobody can recover them), and changing the salt would orphan every stored secret, which is why `POST /vault` refuses to run twice. The residual risk is inherent to browser vaults: whoever controls the server can serve modified JS that captures the passphrase — the mitigation is server access control, not cryptography.
