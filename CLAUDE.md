...
- `/admin` — case management panel (see «Admin panel» below). Shipped in the export but `noindex` + disallowed in `robots.ts` and deliberately absent from `sitemap.ts`.
