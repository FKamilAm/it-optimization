...
export async function telegramRoutes(app: FastifyInstance): Promise<void> {
  app.get("/telegram/status", { preHandler: requireAuth }, async (request, reply) => {
  ...
  app.post("/telegram/link-code", { preHandler: requireAuth }, async (request, reply) => {
    if (!canNotify) {
  ...
  app.post("/telegram/unlink", { preHandler: requireAuth }, async (request, reply) => {
    await prisma.user.update({
      where: { id: request.user!.id },
